...
基于区块链技术的地理空间数据存证系统，利用 Sepolia 测试网实现数据的哈希存证、时间戳记录和防篡改验证。

项目背景

在地理信息系统（GIS）领域，数据来源的可靠性和时间戳的权威性至关重要。本项目通过智能合约将文件哈希上传至以太坊 Sepolia 测试网，利用区块链的不可篡改特性，为地理空间数据提供公开、可验证的存在证明。
...
pip install web3
3. 配置敏感信息（重要）
在项目根目录创建 config.py 文件（该文件已加入 .gitignore，不会上传到 GitHub），内容如下：
...
篡改检测测试
修改 data/elevation_sample.csv 中的任意数据

再次运行 python verify_hash.py
